...
%             HH=(N'*H(t(1:4,i))');
            ...
            A_i=HH3*(1/B.*dNx'*dNx+B*(dNz'*dNz)+(dNz'*N'))*abs(det(J))*W(j);%toa do cuc
            dA_dhp_i=3*HH2*(B*sin(-theta_p + A)*sin(alpha_r) + sin(alpha_theta)*cos(-theta_p + A)*B - sin(alpha_theta)*r_p)/hp^2*(1/B.*dNx'*dNx+B*(dNz'*dNz)+(dNz'*N'))*abs(det(J))*W(j);
            dA_dalpha_r_i=3*HH2*-B*sin(-theta_p + A)*cos(alpha_r)/hp*(1/B.*dNx'*dNx+B*(dNz'*dNz)+(dNz'*N'))*abs(det(J))*W(j);
            dA_dalpha_theta_i=3*HH2*(r_p - B*cos(-theta_p + A))*cos(alpha_theta)/hp*(1/B.*dNx'*dNx+B*(dNz'*dNz)+(dNz'*N'))*abs(det(J))*W(j);
...
%            Bj=-6*nuy*omega/h2^2*N*-(h1 - h2)*B^2/((A^2 + B^2)*h2*beta)*abs(det(J))*W(j);
            ...
            B_i=-6*nuy*omega*N*( -B*cos(theta_p-A)*sin(alpha_r) +(r_p-B*sin(theta_p-A))*sin(alpha_theta) )*abs(det(J))*W(j)/hp^3;
            dB_dhp_i=-6*nuy*omega*N*3*B*(sin(alpha_r)*cos(-theta_p + A) - sin(alpha_theta)*sin(-theta_p + A))/hp^4*abs(det(J))*W(j);
            dB_dalpha_r_i=-6*nuy*omega*N*-B*cos(-theta_p + A)*cos(alpha_r)/hp^3*abs(det(J))*W(j);
            dB_dalpha_theta_i=-6*nuy*omega*N*B*sin(-theta_p + A)*cos(alpha_theta)/hp^3*abs(det(J))*W(j);
...
D=[dFz_dapha_r dFz_dapha_theta;dX_dapha_r dX_dapha_theta;dY_dapha_r dY_dapha_theta];
new_variables=[alpha_r;alpha_theta]-D\[Fz-Wz;XcF-X_p;YcF-Y_p]
...
figure(3)
plot(0:m,mtp(m/2+1,1:n+1),0:m,mtp(m/2+13,1:n+1),0:m,mtp(m/2-15,1:n+1))
%% 
% figure(1)
