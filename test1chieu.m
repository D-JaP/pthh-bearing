...
omega=30*60/60*2*pi;%toc do goc (rad/s)
...
for i=1:n
    ...
    N=[N1 ;N2 ;N3 ;N4];
    dN_xieta=[ -1/4*(1-eta) -1/4*(1-xi);...
            1/4*(1-eta) -1/4*(1+xi);...
            1/4*(1+eta)  1/4*(1+xi);...
           -1/4*(1+eta)  1/4*(1-xi)];
    J=Jctugiac(xP,yP,eta,xi);
    Jn=inv(J);
    dNx(1:4)=Jn(1,1)*dN_xieta(1:4,1)+Jn(1,2)*dN_xieta(1:4,2);
    dNz(1:4)=Jn(2,1)*dN_xieta(1:4,1)+Jn(2,2)*dN_xieta(1:4,2);
    ...
    vt_S(t(j,i),1:4)=vt_S(t(j,i),1:4) + SS';
    vt_R(t(j,i),1:4)=vt_R(t(j,i),1:4) + RR';
    end
end
