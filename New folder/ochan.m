...
%             HH2=(N'*H(t(1:4,i))')^2;
%             HH=(N'*H(t(1:4,i))');
            HH=1 + (h1 - h2)*(1 - A/beta)/h2;
            HH2=HH^2;
            HH3=HH^3;
            %tinh cac ma tran do cung
            Ai=HH3*(1/B.*dNx'*dNx+B*(dNz'*dNz))*abs(det(J))*W(j);%toa do cuc
%             Ai=(HH3*(1/B.*dNx'*dNx+B*(dNz'*dNz)+(N*dNz)))*abs(det(J))*W(j);%toa do cuc
...
%             Bj=-6*nuy*omega*N*B*-(h1 - h2)/h2/beta*abs(det(J))*W(j)/h2^2;

            Bj=-6*nuy*omega*N*B*(dNx*H(t(1:4,i))')*abs(det(J))*W(j)/h2^2;
...
b(I_boundry)=0;
% bh(I_boundry)=0;
vtp=(mtA_d\b)';% tinh vecto ap suat
...
figure(3)
plot(m:-1:0,mtp(m/2+1,1:n+1)*h2^2/(12*pi*1500/60*0.039*r2^2))
hold on
x=[0.03589 0.06153 0.102564 0.174359 0.251282 0.369231 0.517949 0.625641 0.453846 0.94359 1.11282 1.28718 1.50256 1.76923 2.01538 2.32308 2.72308];
y=[0.0014087 0.00276522 0.00443478 0.00652174 0.00829565 0.0100435 0.0108522 0.0109565 0.0106435 0.00986087 0.00894783 0.00795652 0.00675652 0.00534783 0.00417391 0.00281739 0.00117391];
x1=x(length(x):-1:1);
