...
%             h(i,j)=(hp+p(2,luoi(i,j))*sin(theta_p-p(1,luoi(i,j)))*sin(alpha_r)+(r_p-p(2,luoi(i,j))*cos(theta_p-p(1,luoi(i,j))))*sin(alpha_theta))/hp;
            h(i,j)=(hp+p(2,luoi(i,j))*sin(-p(1,luoi(i,j)))*sin(alpha_r)+(r_p-p(2,luoi(i,j))*cos(-p(1,luoi(i,j))))*sin(alpha_theta))/hp;
...
%             HH=(N'*H(t(1:4,i))');
            ...
            dA_dalpha_r_i=3*HH2*-C*cos(alpha_r)/hp*(dNx'*dNx+(dNz'*dNz))*abs(det(J))*W(j);
            dA_dalpha_theta_i=3*HH2*(r_p - D)*cos(alpha_theta)/hp*(dNx'*dNx+(dNz'*dNz))*abs(det(J))*W(j);
...
%             B_i=-6*nuy*omega*N*B*(dNx*H(t(1:4,i))'*cos(A)-dNz*H(t(1:4,i))'*sin(A))*abs(det(J))*W(j);
            dh_dx=-sin(alpha_r)/hp;
            dh_dy=-sin(alpha_theta)/hp;
...
%             B_y_i=sqrt(C^2+D^2)*C*(sin(theta_p - atan(C/D))*sin(alpha_r)*C - sin(theta_p - atan(C/D))*sin(alpha_theta)*D - cos(theta_p - atan(C/D))*sin(alpha_r)*D - cos(theta_p - atan(C/D))*sin(alpha_theta)*C)/((C^2 + D^2)*hp);
            ...
            dB_dalpha_theta_i=-6*nuy*omega*N*sqrt(C^2+D^2)*C*cos(alpha_theta)/(sqrt(C^2 + D^2)*hp)*abs(det(J))*W(j);
            dB_dalpha_r_i=-6*nuy*omega*N*-sqrt(C^2+D^2)*cos(alpha_r)*D/(sqrt(C^2 + D^2)*hp)*abs(det(J))*W(j);
