...
%             h(i,j)=(1 + delta*(1 - acot(pp(2,luoi(i,j))/pp(1,luoi(i,j)))/sqrt(pp(1,luoi(i,j))^2 + pp(2,luoi(i,j))^2)));
            ...
            h2(i,j)=1 + sqrt(pp(1,luoi(i,j))^2 + pp(2,luoi(i,j))^2)*sin(theta_p - atan(pp(1,luoi(i,j))/pp(2,luoi(i,j))))*sin(alpha_r)/hp + (r_p - sqrt(pp(1,luoi(i,j))^2 + pp(2,luoi(i,j))^2)*cos(theta_p - atan(pp(1,luoi(i,j))/pp(2,luoi(i,j)))))*sin(alpha_theta)/hp;
...
%             HH=(N'*H(t(1:4,i))');
            ...
            dA_dalpha_r_i=3*HH2*sqrt(C^2 + D^2)*sin(theta_p - atan(C/D))*cos(alpha_r)/hp*(dNx'*dNx+(dNz'*dNz))*abs(det(J))*W(j);
            dA_dalpha_theta_i=3*HH2*(r_p - sqrt(C^2 + D^2)*cos(theta_p - atan(C/D)))*cos(alpha_theta)/hp*(dNx'*dNx+(dNz'*dNz))*abs(det(J))*W(j);
...
%             B_i=-6*nuy*omega*N*B*(dNx*H(t(1:4,i))'*cos(A)-dNz*H(t(1:4,i))'*sin(A))*abs(det(J))*W(j);
...
%             B_y_i=sqrt(C^2+D^2)*C*(sin(theta_p - atan(C/D))*sin(alpha_r)*C - sin(theta_p - atan(C/D))*sin(alpha_theta)*D - cos(theta_p - atan(C/D))*sin(alpha_r)*D - cos(theta_p - atan(C/D))*sin(alpha_theta)*C)/((C^2 + D^2)*hp);
            B_x_i=cos(A)*dNx*H(t(1:4,i))';
            B_y_i=sin(A)*dNz*H(t(1:4,i))';
            
            B_i=-6*nuy*omega*N*sqrt(C^2+D^2)*(B_x_i-B_y_i)*abs(det(J))*W(j);
            
            dB_dalpha_theta_i=-6*nuy*omega*N*(-2*sin(theta_p - atan(C/D))*C*D - cos(theta_p - atan(C/D))*C^2 + cos(theta_p - atan(C/D))*D^2)*sqrt(C^2+D^2)*cos(alpha_theta)/((C^2 + D^2)*hp)*abs(det(J))*W(j);
            dB_dalpha_r_i=-6*nuy*omega*N*((C^2 - D^2)*sin(theta_p - atan(C/D)) - 2*cos(theta_p - atan(C/D))*C*D)*sqrt(C^2+D^2)*cos(alpha_r)/((C^2 + D^2)*hp)*abs(det(J))*W(j);
...
figure(3)
plot(0:m,mtp(m/2+1,1:n+1),0:m,mtp(m*0.8+1,1:n+1),0:m,mtp(m*0.15+3,1:n+1))

% toadox(luoi(:,:))=pp(1,luoi(:,:));
% toadoy(luoi(:,:))=pp(2,luoi(:,:));
% plot(toadox(),toadoy(),'-')
% axis equal
%% 
% figure(1)
