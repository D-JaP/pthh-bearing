...
%%    
    ...
    for i=1:spt
        ...
        for j=1:9
            ...
            dA_dhp_i=(B*sin(-theta_p + A)*sin(alpha_r) +sin(alpha_theta)*cos(-theta_p + A)*B - sin(alpha_theta)*r_p)/hp^2*(3*HH2*(1/B.*dNx'*dNx+B*(dNz'*dNz)))*B*abs(det(J))*W(j);
            dA_dalpha_r_i=               -B*sin(-theta_p + A)*cos(alpha_r)/hp*(3*HH2*(1/B.*dNx'*dNx+B*(dNz'*dNz)))*B*abs(det(J))*W(j);
            dA_dalpha_theta_i=(r_p - B*cos(-theta_p + A))*cos(alpha_theta)/hp*(3*HH2*(1/B.*dNx'*dNx+B*(dNz'*dNz)))*B*abs(det(J))*W(j);
...
%             Ah=3*HH2*-(beta - acot(B/A))*h1/(h2^2*beta)*(dNx'*dNx+(dNz'*dNz))*abs(det(J))*W(j); 
            ...
            B_i=-6*nuy*omega*N*B*B*(sin(alpha_theta)*sin(-theta_p + A) - sin(alpha_r)*cos(-theta_p + A))/hp^3*B*abs(det(J))*W(j);
...
dX_dhp=1/Fz.*(vt_X'*dp_dhp-dFz_dhp*Xc_tinh);
dY_dhp=1/Fz.*(vt_Y'*dp_dhp-dFz_dhp*Yc_tinh);
...
Wz=50000;
DD=[dFz_dhp dFz_dapha_r dFz_dapha_theta;dX_dhp dX_dapha_r dX_dapha_theta;dY_dhp dY_dapha_r dY_dapha_theta];
new_variables=[hp;alpha_r;alpha_theta]-DD\[Fz-Wz;Xc_tinh-X_p;Yc_tinh-Y_p];
...
figure(3)
plot(0:m,mtp(m/2+1,1:n+1),0:m,mtp(m*0.8+1,1:n+1),0:m,mtp(m*0.2+1,1:n+1))
figure(4)
