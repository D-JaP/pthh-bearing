...
omega=2000/60*(2*pi);%toc do goc (rad/s)
...
%%    
    ...
    R2sintheta=zeros(snut,1);
    RpRcostheta=zeros(snut,1);
    %tich phan so Gauss
...
%             B_i=-6*nuy*omega*N*B*B*(-sin(alpha_r)*cos(-theta_p + A) + sin(alpha_theta)*sin(-theta_p + A))/hp^3*abs(det(J))*W(j);
            B_i=-6*nuy*omega*N*B^2*(sin(alpha_theta)*sin(-theta_p + A) - sin(alpha_r)*cos(-theta_p + A))/hp*abs(det(J))*W(j);
            
            dB_dalpha_r_i=-6*nuy*omega*N*B*(-B*cos(-theta_p + A)*cos(alpha_r)/hp^3)*abs(det(J))*W(j);
            dB_dalpha_theta_i=-6*nuy*omega*N*B*(B*sin(-theta_p + A)*cos(alpha_theta)/hp^3)*abs(det(J))*W(j);
...
%             bhi=-6*nuy*omega*N*B*(3*h1 - 2*h2)/(sqrt(A^2 + B^2)*h2^4*beta)*abs(det(J))*W(j);
            ...
            R2sintheta_i=N*B^2*sin(theta_p - A)*abs(det(J))*W(j);
            R2sintheta(t(1:4,i))=R2sintheta(t(1:4,i)) + R2sintheta_i;
            RpRcostheta_i=N*B*(r_p-B)*cos(theta_p - A)*abs(det(J))*W(j);
            RpRcostheta(t(1:4,i))=RpRcostheta(t(1:4,i)) + RpRcostheta_i;
...
D=[R2sintheta'*dp_dalpha_r R2sintheta'*dp_dalpha_theta;...
    RpRcostheta'*dp_dalpha_r RpRcostheta'*dp_dalpha_theta];
% D=[dX_dapha_r dX_dapha_theta;dY_dapha_r dY_dapha_theta];
% new_variables=[alpha_r;alpha_theta]-D\[Xc_tinh-theta_p;Yc_tinh-r_p]
new_variables=[alpha_r;alpha_theta]-D\[M_r;M_theta]
alpha_r_new=new_variables(1);
alpha_theta_new=new_variables(2);
% (Xc_tinh-theta_p)/Xc_tinh
% (Yc_tinh-r_p)/Yc_tinh
...
if (         norm(alpha_r_new-alpha_r)/norm(alpha_r_new)<=1e-5&&...
        ...
        M_r<=1e-5&&M_theta<=1e-5)
%         norm((Xc_tinh-theta_p)/norm(Xc_tinh))<=1e-5&&...
%         norm((Yc_tinh-r_p)/norm(Yc_tinh))<=1e-5)
    disp('done')
break
end
thu=thu+1;
