...
%     end
    ...
    for i=1:m+1
        for j=1:n
            ...
            if (i~=1&&i~=m+1&&sum(j==bien_cd)==0)
                
                I_active(q)=luoi(i,j);
                q=q+1;
                
            elseif (i==1||i==m+1||sum(j==bien_cd)~=0)
...
%%
% figure(8)
Kxx(ll)=6*nuy*omega*R^2*(R/c)^2*D_jc(1,1)/(Wx*6*nuy*omega*(R/c)^2*R*R);
Kxy(ll)=6*nuy*omega*R^2*(R/c)^2*D_jc(1,2)/(Wx*6*nuy*omega*(R/c)^2*R*R);
Kyx(ll)=6*nuy*omega*R^2*(R/c)^2*D_jc(2,1)/(Wx*6*nuy*omega*(R/c)^2*R*R);
Kyy(ll)=6*nuy*omega*R^2*(R/c)^2*D_jc(2,2)/(Wx*6*nuy*omega*(R/c)^2*R*R);
