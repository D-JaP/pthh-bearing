...
Wy=0;Wx=0.07;%Wx=70/(6*nuy*omega*(R/c)^2*R*L);
...
for i=1:spt
    ...
    %tinh vecto tai
    Bj=N*(x*sin(N'*phi(chiso_h(t(1:4,i)))')-y*cos(N'*phi(chiso_h(t(1:4,i)))'))*abs(det(J));
...
pxy=Am(Ia,Ia)\[-mtAmx(Ia,Ia)*vtp(Ia)+bmx(Ia) -mtAmy(Ia,Ia)*vtp(Ia)+bmy(Ia)];
pnx=zeros(snut,1);
pny=zeros(snut,1);
pnx(Ia)=pxy(:,1);
pny(Ia)=pxy(:,2);

D_jc=-[vt_S(Ia)'*pnx(Ia) vt_S(Ia)'*pny(Ia);...
        vt_R(Ia)'*pnx(Ia) vt_R(Ia)'*pny(Ia)];
...
thu=thu+1;
if ((norm(u_new-u)/norm(u_new))<=1e-4&&(norm([fx;fy]-[Wx;Wy])/norm([Wx;Wy]))<1e-4)
    disp('done')
break
end
end
