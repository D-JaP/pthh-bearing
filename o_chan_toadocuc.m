...
omega=400/60*(2*pi);%toc do goc (rad/s)
...
%%
    ...
    while(1)
        ...
        h2=0.0000075;
        h1=h2+0.035*beta*tan(0.14/180*pi);
%         h1=2.75*h2;
...
%             HH=(N'*H(t(1:4,i))');
            ...
            Ai=HH3*(1/B.*dNx'*dNx+B*(dNz'*dNz))*B*abs(det(J))*W(j);%toa do cuc
%             Ai=(HH3*(1/B.*dNx'*dNx+B*(dNz'*dNz)+(N*dNz)))*abs(det(J))*W(j);%toa do cuc


            Ah=3*HH2*-(beta - A)*h1/(h2^2*beta)*(1/B.*dNx'*dNx+B*(dNz'*dNz))*B*abs(det(J))*W(j);
...
%             Bj=-6*nuy*omega*N*B*-(h1 - h2)/h2/beta*abs(det(J))*W(j)/h2^2;
            ...
            Bj=-6*nuy*omega*N*B*(dNx*H(t(1:4,i))')*B*abs(det(J))*W(j);
            bhi=-6*nuy*omega*N*B*h1/(h2^2*beta)*B*abs(det(J))*W(j);
...
b(I_boundry)=0;
bh(I_boundry)=0;
vtp=(mtA_d\b)'/h2^2;% tinh vecto ap suat
...
h2_new=h2-D\(Fz-Wz)
if ((norm(h2_new-h2)/norm(h2_new))<=1e-5&&(norm(Fz-Wz)/norm(Wz))<1e-5)
    disp('done')
break
end
thu=thu+1;
